...
namespace Laboratory_Management_System.UserControls.AdminControls.Crud
{
    public partial class EditSchedule : Form
    {
        ...
        public EditSchedule(string scheduleId)
        {
            InitializeComponent();
            ...
            LoadScheduleData();  // Call this to load schedule data
            ...
            var rooms = new List<string>
    {
        ...
        {
            string query = "SELECT instructor, course, subject, year, block, day, class_start, class_end, room FROM schedules WHERE schedule_id = @ScheduleId";
        ...
        new MySqlParameter("@ScheduleId", scheduleId)
            ...
            try
            {
                DataTable dataTable = dbHelper.GetData(query, parameters);

                if (dataTable.Rows.Count > 0)
                {
                    DataRow row = dataTable.Rows[0];

                    instructorDropdown.SelectedValue = row["instructor"].ToString();
                    courseDropdown.SelectedItem = row["course"].ToString();
                    subjectDropdown.SelectedItem = row["subject"].ToString();
                    yearDropdown.SelectedItem = row["year"].ToString();
                    blockDropdown.SelectedItem = row["block"].ToString();
                    dayDropdown.SelectedItem = row["day"].ToString();
                    classStart.Value = DateTime.Parse(row["class_start"].ToString());
                    classEnd.Value = DateTime.Parse(row["class_end"].ToString());
                    roomDropdown.SelectedItem = row["room"].ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        private void update_Click(object sender, EventArgs e)
        {
            ...
            string query = "UPDATE `schedules` SET `instructor` = @Instructor, `course` = @Course, `subject` = @Subject, `year` = @Year, " +
                           "`block` = @Block, `day` = @Day, `class_start` = @ClassStart, `class_end` = @ClassEnd, `room` = @Room " +
